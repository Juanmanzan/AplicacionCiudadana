...
function prettyGenero(g) {
  ...
  if (s === 'H' || s === 'MASCULINO') return 'Masculino';
  if (s === 'M' || s === 'FEMENINO') return 'Femenino';
  return g ?? 'No registrado';
}

// ✅ icono base (el que ya tienes con tu CSS)
function buildIcon(tipoId, origen = 'REPORTE') {
  const cfg = cfgTipo(tipoId);
  const isAlerta = String(origen).toUpperCase() === 'ALERTA';

  const html = `
    <div class="flat-marker ${isAlerta ? 'is-alerta' : ''}" style="--c:${cfg.color}">
      <i class="bi ${cfg.bi}"></i>
    </div>
  `;
...
function popupHtml(e) {
  const origen = (e.origen || 'REPORTE').toUpperCase();
  const origenBadge = origen === 'ALERTA'
    ? '<span class="emg-chip emg-alerta">ALERTA</span>'
    : '<span class="emg-chip">REPORTE</span>';
...
function upsertMarker(e) {
  ...
  if (!markersById[id]) {
    const marker = L.marker(
      [e.lat, e.lng],
      { icon: buildIcon(e.id_tipo_emergencia, e.origen) }
    )
  ...
  m.setIcon(buildIcon(e.id_tipo_emergencia, e.origen));
...
function bindWsDebug() {
  try {
    ...
    console.log('WS state inicial:', conn.state);

    conn.bind('state_change', (s) => console.log('🔁 WS state_change:', s));
    conn.bind('connected', () => console.log('✅ WS conectado (evento)'));
    conn.bind('error', (err) => console.log('❌ WS error', err));

    if (conn.state === 'connected') {
      console.log('✅ WS conectado (ya estaba conectado)');
    }
  } catch (e) {
    console.error('❌ bindWsDebug error:', e);
  }
}
